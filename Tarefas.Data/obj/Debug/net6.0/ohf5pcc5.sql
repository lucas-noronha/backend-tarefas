CREATE TABLE IF NOT EXISTS "__EFMigrationsHistory" (
    "MigrationId" character varying(150) NOT NULL,
    "ProductVersion" character varying(32) NOT NULL,
    CONSTRAINT "PK___EFMigrationsHistory" PRIMARY KEY ("MigrationId")
);

START TRANSACTION;


DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221030030706_Primeira_migracao') THEN
    CREATE TABLE clientes (
        id uuid NOT NULL,
        bairro text NOT NULL,
        cidade text NOT NULL,
        uf character varying(2) NOT NULL,
        logradouro text NOT NULL,
        numero text NOT NULL,
        cep text NOT NULL,
        nome text NOT NULL,
        data_criacao timestamp with time zone NOT NULL,
        CONSTRAINT "PK_clientes" PRIMARY KEY (id)
    );
    END IF;
END $EF$;

DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221030030706_Primeira_migracao') THEN
    CREATE TABLE usuarios (
        id uuid NOT NULL,
        login text NOT NULL,
        senha text NOT NULL,
        tipo_usuario integer NOT NULL,
        nome text NOT NULL,
        data_criacao timestamp with time zone NOT NULL,
        CONSTRAINT "PK_usuarios" PRIMARY KEY (id)
    );
    END IF;
END $EF$;

DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221030030706_Primeira_migracao') THEN
    CREATE TABLE chamados (
        id uuid NOT NULL,
        titulo text NOT NULL,
        descricao text NOT NULL,
        data_prevista timestamp with time zone NOT NULL,
        data_criacao timestamp with time zone NOT NULL,
        tipo integer NOT NULL,
        id_criador uuid NOT NULL,
        id_responsavel uuid NOT NULL,
        id_cliente uuid NOT NULL,
        status integer NOT NULL,
        CONSTRAINT "PK_chamados" PRIMARY KEY (id),
        CONSTRAINT "FK_chamados_clientes_id_cliente" FOREIGN KEY (id_cliente) REFERENCES clientes (id) ON DELETE CASCADE,
        CONSTRAINT "FK_chamados_usuarios_id_criador" FOREIGN KEY (id_criador) REFERENCES usuarios (id) ON DELETE CASCADE,
        CONSTRAINT "FK_chamados_usuarios_id_responsavel" FOREIGN KEY (id_responsavel) REFERENCES usuarios (id) ON DELETE CASCADE
    );
    END IF;
END $EF$;

DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221030030706_Primeira_migracao') THEN
    CREATE TABLE historicos_chamados (
        id uuid NOT NULL,
        anotacao text NOT NULL,
        id_usuario uuid NOT NULL,
        data_ocorrencia timestamp with time zone NOT NULL,
        id_chamado uuid NOT NULL,
        CONSTRAINT "PK_historicos_chamados" PRIMARY KEY (id),
        CONSTRAINT "FK_historicos_chamados_chamados_id_chamado" FOREIGN KEY (id_chamado) REFERENCES chamados (id) ON DELETE CASCADE,
        CONSTRAINT "FK_historicos_chamados_usuarios_id_usuario" FOREIGN KEY (id_usuario) REFERENCES usuarios (id) ON DELETE CASCADE
    );
    END IF;
END $EF$;

DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221030030706_Primeira_migracao') THEN
    CREATE TABLE tempo_gasto (
        id uuid NOT NULL,
        tempo_gasto interval NOT NULL,
        atividade text NOT NULL,
        data_criacao timestamp with time zone NOT NULL,
        id_chamado uuid NOT NULL,
        CONSTRAINT "PK_tempo_gasto" PRIMARY KEY (id),
        CONSTRAINT "FK_tempo_gasto_chamados_id_chamado" FOREIGN KEY (id_chamado) REFERENCES chamados (id) ON DELETE CASCADE
    );
    END IF;
END $EF$;

DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221030030706_Primeira_migracao') THEN
    CREATE INDEX "IX_chamados_id_cliente" ON chamados (id_cliente);
    END IF;
END $EF$;

DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221030030706_Primeira_migracao') THEN
    CREATE INDEX "IX_chamados_id_criador" ON chamados (id_criador);
    END IF;
END $EF$;

DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221030030706_Primeira_migracao') THEN
    CREATE INDEX "IX_chamados_id_responsavel" ON chamados (id_responsavel);
    END IF;
END $EF$;

DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221030030706_Primeira_migracao') THEN
    CREATE INDEX "IX_historicos_chamados_id_chamado" ON historicos_chamados (id_chamado);
    END IF;
END $EF$;

DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221030030706_Primeira_migracao') THEN
    CREATE INDEX "IX_historicos_chamados_id_usuario" ON historicos_chamados (id_usuario);
    END IF;
END $EF$;

DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221030030706_Primeira_migracao') THEN
    CREATE INDEX "IX_tempo_gasto_id_chamado" ON tempo_gasto (id_chamado);
    END IF;
END $EF$;

DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221030030706_Primeira_migracao') THEN
    INSERT INTO "__EFMigrationsHistory" ("MigrationId", "ProductVersion")
    VALUES ('20221030030706_Primeira_migracao', '6.0.10');
    END IF;
END $EF$;
COMMIT;

START TRANSACTION;


DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221102210605_Adicionando_coluna_inativo') THEN
    ALTER TABLE usuarios ADD inativo boolean NOT NULL DEFAULT FALSE;
    END IF;
END $EF$;

DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221102210605_Adicionando_coluna_inativo') THEN
    ALTER TABLE clientes ADD inativo boolean NOT NULL DEFAULT FALSE;
    END IF;
END $EF$;

DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221102210605_Adicionando_coluna_inativo') THEN
    INSERT INTO "__EFMigrationsHistory" ("MigrationId", "ProductVersion")
    VALUES ('20221102210605_Adicionando_coluna_inativo', '6.0.10');
    END IF;
END $EF$;
COMMIT;

START TRANSACTION;


DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221107233052_Vendo_relacoes') THEN
    ALTER TABLE usuarios ALTER COLUMN data_criacao TYPE timestamp without time zone;
    END IF;
END $EF$;

DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221107233052_Vendo_relacoes') THEN
    ALTER TABLE tempo_gasto ALTER COLUMN data_criacao TYPE timestamp without time zone;
    END IF;
END $EF$;

DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221107233052_Vendo_relacoes') THEN
    ALTER TABLE historicos_chamados ALTER COLUMN data_ocorrencia TYPE timestamp without time zone;
    END IF;
END $EF$;

DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221107233052_Vendo_relacoes') THEN
    ALTER TABLE clientes ALTER COLUMN data_criacao TYPE timestamp without time zone;
    END IF;
END $EF$;

DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221107233052_Vendo_relacoes') THEN
    ALTER TABLE chamados ALTER COLUMN data_prevista TYPE timestamp without time zone;
    END IF;
END $EF$;

DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221107233052_Vendo_relacoes') THEN
    ALTER TABLE chamados ALTER COLUMN data_criacao TYPE timestamp without time zone;
    END IF;
END $EF$;

DO $EF$
BEGIN
    IF NOT EXISTS(SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20221107233052_Vendo_relacoes') THEN
    INSERT INTO "__EFMigrationsHistory" ("MigrationId", "ProductVersion")
    VALUES ('20221107233052_Vendo_relacoes', '6.0.10');
    END IF;
END $EF$;
COMMIT;

